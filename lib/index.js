// var EPub, Q, _, archiver, cheerio, ejs, entities, fs, fsextra, mime, path, removeDiacritics, request, rimraf, uslug, uuid, indexOf = [].indexOf;
  ...
  constructor(options, output) {
    ...
    this.options.content = _.map(this.options.content, function(content, index) {
      ...
      if (!content.filename) {
        ...
        content.sourceDir = path.dirname(content.filename);
        content.href = `${path.basename(content.filename, path.extname(content.filename))}.xhtml`;
        content.filePath = path.resolve(__dirname, `${self.uuid}/OEBPS/${content.href}`);
        content.data = fsextra.readFileSync(content.filename, {encoding: 'utf8'});
      }
      ...
      $("img").each(function(index, elem) {
        ...
        if (image = self.options.images.find(function(element) {
          ...
          dir = content.dir;
          var sourceDir = content.sourceDir;
          self.options.images.push({id, url, sourceDir, dir, mediaType, extension});
        }
  ...
  downloadImage(options) { //{id, url, mediaType}
    ...
    if (options.url.indexOf("file://") === 0) {
      ...
      if (options.url.indexOf("http") === 0) {
        ...
        const imagePath = path.resolve(options.sourceDir, options.url);
        console.log('>>>', imagePath)
        requestAction = fs.createReadStream(imagePath);
        requestAction.pipe(fs.createWriteStream(filename));
      }
